...
FIREBASE_CLIENT_CONFIG = None # Variável para guardar a config do frontend
...
try:
    ...
    FIREBASE_CLIENT_CONFIG_STR = os.environ.get('FIREBASE_CLIENT_CONFIG') # Carrega config do cliente
...
# Tenta carregar a configuração do cliente para injeção no frontend
if FIREBASE_CLIENT_CONFIG_STR:
    try:
        FIREBASE_CLIENT_CONFIG = json.loads(FIREBASE_CLIENT_CONFIG_STR)
        print("INFO: Configuração do Firebase Client carregada.")
    except json.JSONDecodeError:
        print("ERRO: Variável FIREBASE_CLIENT_CONFIG não é um JSON válido.")
        FIREBASE_CLIENT_CONFIG = None
...
def gerar_certificado():
    ...
    # OBS: A função generate_latex_certificate não está definida neste snippet,
    # mas a lógica de roteamento aqui está correta.
    # latex_content = generate_latex_certificate(nome_completo, data_conclusao_str, carga_horaria)
    latex_content = "Conteúdo de certificado LaTeX de exemplo"
...
def conteudo_dinamico(modulo_slug):
    ...
    # =========================================================================
    # INJEÇÃO DA CONFIGURAÇÃO DO FIREBASE CLIENT E TOKEN
    # =========================================================================
    
    # Adicionando a injeção da config do cliente (agora carregada no início do arquivo)
    extra_context['__firebase_config'] = FIREBASE_CLIENT_CONFIG 
    
    # MANTIDO: A injeção do token de autenticação e do APP_ID
    # O token DEVE ser gerado dinamicamente no backend.
    # OBS: Pode falhar se o Firebase Admin não foi inicializado (credenciais ausentes)
    try:
        extra_context['__initial_auth_token'] = auth.create_custom_token(user_id).decode()
    except Exception as e:
        print(f"AVISO: Não foi possível criar o token de autenticação. Erro: {e}")
        extra_context['__initial_auth_token'] = '' # Garante que o frontend não quebre
